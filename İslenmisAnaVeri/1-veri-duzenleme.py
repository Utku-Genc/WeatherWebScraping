...
df = pd.read_csv("AnaVeri/weather_data_merged.csv")
...
df["Temperature"] = df["Temperature"].apply(lambda x: f"{fahrenheit_to_celsius(float(x.split()[0])):.2f}" if "°F" in x else x)
df["Dew Point"] = df["Dew Point"].apply(lambda x: f"{fahrenheit_to_celsius(float(x.split()[0])):.2f}" if "°F" in x else x)
...
df.rename(columns={
    "Date": "Date (Tarih)",
    "Time": "Time (Saat)",
    "Temperature": "Temperature (Sıcaklık) (°C)",
    "Dew Point": "Dew Point (Çiğ Noktası) (°C)",
...
df.to_csv("İslenmisAnaVeri/tr_weather_data.csv", index=False)
